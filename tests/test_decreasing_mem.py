...
class TestDecreasingMembershipFunction(unittest.TestCase):
    ...
    def test_returns_interpolated_value_between_a_and_b(self):
        self.assertAlmostEqual(dec(15, 10, 20), 0.5)
        self.assertAlmostEqual(dec(12, 10, 20), 0.8)
        self.assertAlmostEqual(dec(18, 10, 20), 0.2)
    ...
    def test_handles_negative_ranges(self):
        self.assertEqual(dec(-15, -10, 0), 1)
        self.assertAlmostEqual(dec(-5, -10, 0), 0.5)
        self.assertEqual(dec(0, -10, 0), 0)
    ...
    def test_handles_very_large_values(self):
        self.assertEqual(dec(1000000, 500000, 1000000), 0)
        self.assertEqual(dec(499999, 500000, 1000000), 1)
        self.assertAlmostEqual(dec(750000, 500000, 1000000), 0.5)

    def test_handles_a_equals_b_boundary_case(self):
        self.assertEqual(dec(10, 10, 10), 1)
        self.assertEqual(dec(10.0001, 10, 10), 0)

    def test_handles_x_as_numpy_array(self):
        x_array = np.array([5, 10, 15, 20, 25])
        result = np.array([dec(val, 10, 20) for val in x_array])
        expected = np.array([1, 1, 0.5, 0, 0])
        np.testing.assert_array_almost_equal(result, expected)

    def test_handles_float_precision_at_boundaries(self):
        a, b = 10.0, 20.0
        self.assertAlmostEqual(dec(a - 0.0001, a, b), 1)
        self.assertAlmostEqual(dec(b + 0.0001, a, b), 0)

    def test_produces_continuous_monotonic_decreasing_output(self):
        a, b = 10, 20
        x_values = np.linspace(5, 25, 100)
        y_values = [dec(x, a, b) for x in x_values]

        for i in range(len(y_values) - 1):
            self.assertGreaterEqual(y_values[i], y_values[i + 1])

    def test_handles_single_point_interval_a_equals_b(self):
        self.assertEqual(dec(9.99, 10, 10), 1)
        self.assertEqual(dec(10.00, 10, 10), 1)
        self.assertEqual(dec(10.01, 10, 10), 0)

    def test_returns_exactly_half_at_midpoint(self):
        a, b = 0, 10
        midpoint = (a + b) / 2
        self.assertAlmostEqual(dec(midpoint, a, b), 0.5)

    def test_handles_co2_sensor_range(self):
        a, b = 400, 1500
        self.assertEqual(dec(350, a, b), 1)
        self.assertEqual(dec(1500, a, b), 0)
        self.assertAlmostEqual(dec(950, a, b), (1500 - 950) / (1500 - 400))

    def test_handles_temperature_sensor_range(self):
        a, b = 18, 30
        self.assertEqual(dec(10, a, b), 1)
        self.assertEqual(dec(30, a, b), 0)
        self.assertAlmostEqual(dec(24, a, b), 0.5)

    def test_handles_humidity_sensor_range(self):
        a, b = 30, 80
        self.assertEqual(dec(25, a, b), 1)
        self.assertEqual(dec(80, a, b), 0)
        self.assertAlmostEqual(dec(55, a, b), 0.5)
    ...
    def test_output_is_complement_of_increasing_function(self):
        ...
        for x in test_values:
            self.assertAlmostEqual(dec(x, a, b), 1 - inc(x, a, b))

    def test_correctly_implements_falling_ramp_semantics(self):
        a, b = 0, 100
        self.assertEqual(dec(a - 1, a, b), 1)
        self.assertGreater(dec(a, a, b), 0.99)
        self.assertLess(dec(b, a, b), 0.01)
        self.assertEqual(dec(b + 1, a, b), 0)
